...
const TOTAL_TIME = 35 * 60; // total waktu quiz dalam detik (35 menit)
...
function sendToGoogleSheets(results, timeUp) {
  ...
  const body = new URLSearchParams({
    ...
    total: results.total,
    persen: persen + '%',
    katakerja: `${results.perCategory['K. Kerja'].correct}/${results.perCategory['K. Kerja'].total}`,
    katasifat: `${results.perCategory['K. Sifat'].correct}/${results.perCategory['K. Sifat'].total}`,
    katabenda: `${results.perCategory['K. Benda'].correct}/${results.perCategory['K. Benda'].total}`,
    kataketerangan: `${results.perCategory['K. Keterangan'].correct}/${results.perCategory['K. Keterangan'].total}`,
...
function renderResult(results, timeUp) {
  ...
  document.getElementById('review-toggle-label').textContent = '📋 Tampilkan Review Jawaban';
}
